...
type IndustryPack struct {
	...
	ShareCrossDept int       `gorm:"default:1" json:"share_cross_dept"`
...
type TenantPackBinding struct {
	...
	AppliedVersion   string    `gorm:"size:20" json:"applied_version"`
	// 企业包层（可选；企业=租户的上级资产来源，物化时叠加在行业包之上）
	EnterprisePackID   *uint   `json:"enterprise_pack_id"`
	EnterpriseCode     string  `gorm:"size:32" json:"enterprise_code"`
	EnterpriseVersion  string  `gorm:"size:20" json:"enterprise_version"`
...
type DeptPackBinding struct {
	...
	PackCode       string    `gorm:"size:32" json:"pack_code"`
	AppliedVersion string    `gorm:"size:20" json:"applied_version"`
	BoundAt        time.Time `json:"bound_at"`
	UpdatedAt      time.Time `json:"updated_at"`
}
