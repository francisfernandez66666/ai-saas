...
type TenantPackBinding struct {
	...
	AppliedVersion string `gorm:"size:20" json:"applied_version"` // 已应用版本
	// 企业包层（可选；企业=租户的上级资产来源，物化时叠加在行业包之上）
	EnterprisePackID  *uint     `json:"enterprise_pack_id"`                // 企业包ID
	EnterpriseCode    string    `gorm:"size:32" json:"enterprise_code"`    // 企业包编码
	EnterpriseVersion string    `gorm:"size:20" json:"enterprise_version"` // 企业包版本
...
type DeptPackBinding struct {
	...
	PackID         uint      `gorm:"index" json:"pack_id"`             // 包ID
	PackCode       string    `gorm:"size:32" json:"pack_code"`         // 包编码
	AppliedVersion string    `gorm:"size:20" json:"applied_version"`   // 已应用版本
	BoundAt        time.Time `json:"bound_at"`                         // 绑定时间
	UpdatedAt      time.Time `json:"updated_at"`                       // 更新时间
}
