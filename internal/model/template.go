...
type Template struct {
	...
	PromptTemplate string `gorm:"type:text;not null" json:"prompt_template"` // 抛话术模板
	// HookTemplate 钩话术模板（AI接话后用钩子引导客户回复）
	HookTemplate string `gorm:"type:text" json:"hook_template"` // 钩话术模板
	// HookFields 钩话术需要采集的字段(JSON数组)
	HookFields string `gorm:"type:text" json:"hook_fields"` // 钩采集字段
