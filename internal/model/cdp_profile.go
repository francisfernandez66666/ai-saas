...
type CdpProfile struct {
	ID          uint   `gorm:"primaryKey;autoIncrement"` // 注释：主键ID
	TenantID    uint   `gorm:"default:0;index"` // 注释：租户ID
	CustomerID  uint   `gorm:"index"` // 注释：客户ID
	CdpId       string `gorm:"size:32;uniqueIndex:idx_cdp_profile_tenant_cdpid"` // 注释：CDP实体ID
	ProfileName string `gorm:"size:64"` // 注释：画像名称
	Status      int    `gorm:"default:1"` // 注释：状态
	ProfileData string `gorm:"type:json"` // JSON格式的画像数据
	CreatedAt   time.Time // 注释：创建时间
	UpdatedAt   time.Time // 注释：更新时间
}
...
type CdpTagDefinition struct {
	ID            uint    `gorm:"primaryKey;autoIncrement"` // 注释：主键ID
	TenantID      uint    `gorm:"default:0;index"` // 注释：租户ID
	Code          string  `gorm:"size:32;uniqueIndex:idx_cdp_tag_tenant_code"` // 注释：编码
	Name          string  `gorm:"size:64"` // 注释：名称
	Category      string  `gorm:"size:32"` // 如: intent, behavior, preference
	WeightDefault float64 `gorm:"default:0"` // 注释：默认权重
	IsActive      bool    `gorm:"default:true"` // 注释：是否上架
	CreatedAt     time.Time // 注释：创建时间
	UpdatedAt     time.Time // 注释：更新时间
}
...
type CdpTagAssignment struct {
	ID           uint   `gorm:"primaryKey;autoIncrement"` // 注释：主键ID
	TenantID     uint   `gorm:"default:0;index"` // 注释：租户ID
	CdpProfileID uint   `gorm:"index"` // 注释：关联画像ID
	DefinitionID uint   `gorm:"index"` // 注释：标签定义ID
	TagValue     string `gorm:"size:128"` // 标签的具体值
	AssignedAt   time.Time // 注释：分配时间
