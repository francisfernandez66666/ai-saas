...
func seedFeatures() {
	...
	features := []model.Feature{
		...
		},
		{
			...
			Category:         "服务",
			DescTemplate:     "极石提供整车5年/15万公里质保，三电系统8年/15万公里质保，增程器专项保障更安心。全国服务网点持续扩展中，24小时道路救援随时待命，让你无论城市通勤还是长途穿越都无后顾之忧。",
...
func seedTemplates() {
	...
	templates := []model.Template{
		...
		// 入门破冰：极石品牌介绍，吸引客户关注
		{
			...
			PromptTemplate:   "你好呀~ 欢迎关注极石！我看你对我们的{{model}}挺感兴趣的对吧？极石是做全地形新能源SUV的品牌，增程式动力、全系标配豪华配置，很多像你这样的客户都特别喜欢，现在关注的人越来越多了。",
			HookTemplate:     "请问你是自己开还是家里用呀？平时会开车去户外玩吗？",
		...
		// 周末自驾场景：强调6/7座家庭出行+零重力座椅舒适
		{
			...
			HookTemplate:     "你平时周末会带家人出去玩吗？一般都去什么地方呀？",
		...
		// 越野爱好者场景：ADAMAS的L3越野认证+全地形能力
		{
			...
			RequiredTags:     jsonArr([]string{}),
			PromptTemplate:   "听说你喜欢越野？那你可找对车了！极石ADAMAS就是为越野而生的——双电机全时四驱350kW、272mm离地间隙、770mm涉水深度，中汽研L3级越野认证，7种全地形模式。很多越野圈的朋友都开它去穿越，口碑特别好。",
			HookTemplate:     "你平时都去哪里越野呀？是玩沙漠还是山地？",
		...
		// 同价位推荐：与竞品对比时的通用切入
		{
			...
			RequiredTags:     jsonArr([]string{}),
			PromptTemplate:   "你在对比车型是吧？很正常，买车是大事，多对比对比没错。跟你说，这个价位的新能源SUV里，极石的性价比真的很高——全系标配空气悬架、电吸门、车载冰箱，别的品牌这些都要选装。很多客户对比完最后还是选了极石。",
			HookTemplate:     "你现在在看哪几款车呀？我帮你分析分析~",
		...
		// 动力系统详解：1.5T增程器+双电机+长续航
		{
			...
			RequiredTags:     jsonArr([]string{}),
			MinIntent:        0.2,
			PromptTemplate:   "说到买车，动力肯定是你最关心的吧？我给你好好讲讲极石的动力系统。{{feature_desc}}简单说就是：增程器只发电不直接驱动，效率高还省油，纯电215-306km日常通勤不烧油，综合续航最长1405km长途无焦虑。",
			HookTemplate:     "你平时开车是偏激烈一点还是稳一点呀？对纯电续航和油耗在意吗？",
		...
		// 安全配置详解：全铝车身+智驾+空气悬架
		{
			...
			HookTemplate:     "家里有小朋友吧？极石6/7座布局带老人孩子都方便，你对安全配置还有什么特别关注的吗？",
		...
		// 内饰品质详解：零重力座椅+8155座舱+电吸门+冰箱
		{
			...
			HookTemplate:     "你对内饰的材质和做工要求高吗？有没有特别喜欢的内饰颜色呀？",
		...
		// 规格对比：ADAMAS vs 坦克500/理想L8/方程豹豹8
		{
			...
			MinIntent:        0.4,
			ApplicableModels: jsonArr([]string{"极石ADAMAS"}),
			PromptTemplate:   "说到对比，我给你客观分析一下。看坦克500的话，ADAMAS是承载式全铝车身，公路好开太多了，非承载式大梁车颠得慌；看理想L8的话，ADAMAS能真越野，272mm离地间隙+L3越野认证，L8只是加高的MPV；看方程豹豹8的话，ADAMAS便宜3-7万，配置反而更全——空气悬架、电吸门、车载冰箱全系标配。{{feature_desc}}",
			HookTemplate:     "你对比的那几款车试驾过了吗？实际开起来感觉怎么样？",
		...
		// 服务对比：极石质保+道路救援
		{
			...
			MinIntent:        0.4,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "买车其实只是开始，售后才是关键。极石的服务你完全不用担心，{{feature_desc}}增程器结构简单可靠，保养成本比插混低，这一点很多老客户都特别认可。",
			HookTemplate:     "你之前的车保养维修方便吗？对售后这一块是不是挺看重的？",
		...
		// 性价比对比：全系标配 vs 竞品选装
		{
			...
			MinIntent:        0.3,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "说实话，极石这个价位能买到这个配置的车，真的挺值的。你算一笔账：ADAMAS 34.99万起步，空气悬架、电吸门全车+尾门、8.5L冰箱全系标配；同样的钱买坦克500只能买中配，买理想L8这些配置都得选装。极石01更划算，29.99万起步，增程免购置税再省3万+，落地比同级便宜一大截。",
			HookTemplate:     "你的购车预算大概是多少呀？我帮你算算哪个配置最划算。",
		...
		// 错过优惠：强调极石当前购车权益
		{
			...
			MinIntent:        0.5,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "跟你说个实在话，现在极石的购车权益真的是今年最大的了——金融免息、置换补贴、终身质保选装权，这些权益过了这月就回收了。很多客户就是因为犹豫了一下，后来再买就少了好几万的权益，都挺后悔的。",
			HookTemplate:     "你要是真看好了，这两天定下来最划算。你看这周方便来店里看看吗？",
		...
		// 现车紧张：强调极石热销
		{
			...
			MinIntent:        0.5,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "你看中的这个配置这个颜色，现在现车真的不多了。极石最近卖得特别好，这个月已经走了十几台，库里就剩最后两台了。你要是再等等，说不定就得等下一批车，那得等一两个月呢。",
			HookTemplate:     "你是想提现车还是愿意等呀？现车的话我建议你早点定。",
		...
		// 保养费用：增程比插混/油车更省
		{
			...
			HookTemplate:     "你之前那辆车保养贵吗？一年养车大概花多少钱？",
		...
		// 限量版：ADAMAS专属高配
		{
			...
			MinIntent:        0.6,
			ApplicableModels: jsonArr([]string{"极石ADAMAS"}),
			PromptTemplate:   "跟你说个消息，极石ADAMAS马上要出一个高配限定版，配置拉满还有专属标识和配色，全国配额有限，特别有收藏价值。这个消息我只跟几个重点客户说了，你要是感兴趣我帮你预留个名额。",
			HookTemplate:     "你对限定版这种感兴趣吗？我把配置单发给你看看？",
		...
		// 专属福利：到店专属权益
		{
			...
			MinIntent:        0.6,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "你今天来得真是时候！我们店刚好有个极石店长特批的名额，能额外送你一个大礼包——全车贴膜、脚垫、还有三次保养，加上极石官方的金融免息和置换补贴，综合算下来能省好几万。这个名额一天就一个，你要是今天定我帮你申请。",
			HookTemplate:     "你看这个福利怎么样？今天定下来真的很划算。",
		...
		// 安全代价：全铝车身+智驾不能省
		{
			...
			MinIntent:        0.6,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "我跟你说句掏心窝子的话，买车千万别图便宜省那几万。安全这东西，平时觉得没用，真出事的时候就是保命的。极石全系全铝承载式车身，轻量化高刚性，碰撞安全性远超同级别；高阶版还有3颗激光雷达+508TOPS智驾算力，主动安全拉满。你想想，家人的安全，能用钱衡量吗？",
			HookTemplate:     "你说是不是这个理儿？安全真的不能省。",
		...
		// 品质代价：全系标配豪华不能省
		{
			...
			MinIntent:        0.5,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "很多客户一开始也觉得别的品牌更便宜，但是对比完配置都说极石值。为什么？因为极石全系标配空气悬架、电吸门、车载冰箱，别的品牌这些都要选装加钱。你要是买个看起来便宜的，后面选装加一起反而更贵，配置还不如极石全。这个账你可得算明白。",
			HookTemplate:     "你之前的车开了几年？是不是后期各种加装花了好多钱？",
		...
		// 安抚倾听：低意向客户的温和引导
		{
			...
			MaxIntent:        0.2,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "好的，我明白了。你先慢慢了解极石，有什么问题随时问我。不管你最后买不买，能帮到你我就很开心了。",
			HookTemplate:     "你现在对极石哪方面还想多了解一些呀？",
		...
		// 纯粹解答：最低压力的信息提供
		{
			...
			MaxIntent:        0.15,
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "没问题，你有什么关于极石的问题尽管问，我知道的都告诉你。买车是大事，多了解了解没错的。",
			HookTemplate:     "你现在最关心的是什么问题呀？",
		...
		// 老客户推荐：极石品牌忠诚度
		{
			...
			RequiredTags:     jsonArr([]string{}),
			ApplicableModels: jsonArr([]string{"极石ADAMAS", "极石01"}),
			PromptTemplate:   "你是我们的老客户了，那你肯定知道极石的品质。全铝车身+空气悬架+增程长续航，很多老客户换车还是选极石，就是因为开着放心、用着省心。你这次是想换个更大的ADAMAS还是不同配置的呀？",
			HookTemplate:     "你之前那辆车开了几年了？这次想换个什么价位的？",
		...
		// 四驱系统详解：ADAMAS双电机全时四驱
		{
			...
			HookTemplate:     "你之前开的是两驱还是四驱呀？有没有去试过越野？",
		...
		// 越野能力对比：ADAMAS vs 坦克500/豹8
		{
			...
			MinIntent:        0.4,
			ApplicableModels: jsonArr([]string{"极石ADAMAS"}),
			PromptTemplate:   "你要是喜欢越野，那可得好好比比四驱系统。坦克500是非承载式车身，大梁颠得慌，公路开着像开船；而ADAMAS是全铝承载式+空气悬架，公路舒适越野也硬核。豹8虽然越野参数也不错，但贵了3-7万，配置还不如ADAMAS全。{{feature_desc}}",
			HookTemplate:     "你有没有去试驾过对比的那款车？越野能力怎么样？",
